...
\usepackage{amsmath}
\usepackage{amsfonts}
\usepackage{amssymb}  % https://www.physicsread.com/latex-therefore/
\usepackage{xcolor}
\usepackage{xparse}
...
Here I provide an overview of the Python classes
that encapsulate the attributes (data members) and methods (functions) associated
with the MLP. Since this approach was object-oriented, it was helpful to
...
arguments for $n_x$, $n_h$, and an \codeword{Operation}
that will be used as the vectorized activation function $\phi(\cdot)$ for a given layer.

The \codeword{__init__} method saves the \codeword{Operation} and passes
...
The \codeword{__init__} method takes as arguments the dimension $n_x$ of the input,
...
the cost function $C$ as an \codeword{Operation},
activation function $\phi$ for hidden layers as an \codeword{Operation},
and the activation function for the target layer $\varphi$ as an \codeword{Operation}.
This method
also creates three \codeword{list} objects of particular importance:
...
\codeword{sequential} list is called to compute the weighted inputs $z^{l}$ and
...
input to the \codeword{__call__} method of each \codeword{DenseLayer}.
...
\begin{algorithm}
	...
	\begin{algorithmic}[1]  % Number every line
		...
		\State{$\nabla_{b^{L}} C_{\text{samples}} \gets \text{GradBiases}(A_{L}, Z_{L}, Y)$} \Comment{Bprop eq. 1 $\forall x \in m$}
		...
		\State{$\nabla_{W^{L}} C_{\text{samples}} \gets \text{GradWeights}(A_{L-1}, \nabla_{b^{L}} C_{\text{samples}})$} \Comment{Bprop eq. 4 $\forall x \in m$}
...
The diabetes dataset is available through \codeword{Scikit-Learn}, though the
...
Data was standard scaled using \codeword{Scikit-Learn}.
...
since there is one neuron in the last layer of a network approximating
...
The breast cancer dataset is available through \codeword{Scikit-Learn}, and
...
The input data $X$ was standard scaled using \codeword{Scikit-Learn}.
...
of the \codeword{TensorFlow} reference implementation, which indicates that
...
bounds of the \codeword{TensorFlow} reference implementation, but this is likely
