...
% Make links to pages in table of contents
...
the practical implementation of the four equations for can be discussed.
...
\begin{align}
	Z_{m \times n_h} =
	(W_{\colorvar{blue}{n_h} \times \colorvar{red}{n_x}} (X_{\colorvar{green}{m} \times \colorvar{red}{n_x}})^{\top} + b_{\colorvar{blue}{n_h}})^{\top}
\end{align}
...
returns a matrix $C_{n_h \times m}$ and the vector $b_{n_h}$ is then added
to $C_{n_h \times m}$ such that
$W_{n_h \times n_x}(X_{m \times n_x})^\top + b_{n_h} = C_{n_h \times m} + b_{n_h}$.
...
One can imagine that the column vector $b_{n_h}$ is copied
into adjacent columns until there are $m$ copies of the column vector in
what is now $n_h \times m$ matrix $B$.

The activation matrix $A$ contains the activation vector as a row
for each training example and
is expected to be $A \in \mathbb{R}^{m \times n_h}$. Since the activation matrix
$A$ is
passed onto the next layer as the new input $X$ to the \codeword{__call__} method,
I make sure the matrix is the expected input shape $m \times n_x$. To do this,
I transpose the weighted input matrix to
${Z \in \mathbb{R}^{m \times n_h}}$ so that the activation matrix, which is just
the result of element wise application of the activation function, matches the
expected input for the next layer.
...
the activation function for the target layer $\varphi$ as a string. This method
also creates three \codeword{list} objects of particular important:
\codeword{sequential}, \codeword{activations_cache}, and
\codeword{weighted_inputs_cache}. These \codeword{list} objects are used to
...
\codeword{sequential} list is to compute and the cache weighted inputs and
activations for each layer. Finally, it returns the last activation vector
representing the predictions the model made given the samples.

The \codeword{_backward_pass} is the only pseudocode I will provide since
it is really the only step that cannot figure
