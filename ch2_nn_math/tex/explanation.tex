...
\begin{align}
	...
	\frac{\partial C}{\partial W_{jk}^{l}} & = \delta_{j}^{l} (a_{k}^{l-1})^{\top}
\end{align}
...
function $C$ is for a single example $x$ and $\delta^{L} \in \mathcal{R}^{n_y}$
...
\begin{align}
	...
	\begin{bmatrix}
		...
		\dfrac{\partial C}{\partial a_{2}^{L}}(a_{2}^{L}) \dfrac{\partial \phi}{\partial z}(z_{1}^{L}) \\
		\vdots                                                                                         \\
		\dfrac{\partial C}{\partial a_{(n_{y} - 1)}^{L}}(a_{(n_{y} - 1)}^{L}) \dfrac{\partial \phi}{\partial z}(z_{0}^{L})
	\end{bmatrix}
\end{align}
...
then derivative is easy to compute and shown below.
