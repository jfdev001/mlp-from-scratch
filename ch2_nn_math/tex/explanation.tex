...
where $x$ and $y$ are in $\mathcal{R}^{n_x}$ and
...
\begin{align}
	...
	\frac{\partial C_x}{\partial W_{jk}^{l}} & = \delta_{j}^{l} (a_{k}^{l-1})^{\top}
\end{align}

The error vector $\delta^{L}$ for the last layer $L$ uses the gradient operator
$\nabla_{a^{L}}$ with respect to each activation $a_{j}^{L}$ for the $j^{th}$ neuron.
The gradient for a function of two variables can be succinctly exemplified with
...
\end{align}
That is, the gradient is the vector where each element is the first-order
partial derivative with respect to each variable in the function.

Now, $\nabla_{a^{L}} C$ specifies $a^{L}$ because $C$ can be written as
a function of many parameters and variables. A single layer neural network,
...
of a weight matrix before the input layer.

Consider the activation vector iteslf for a layer $l$ over $j$ neurons as
${a = \set{a_{j}^{l}}_{j=0}^{n_{h} - 1}}$. This is written as
...
If you reduce the problem to the element-by-element case rather than the vector case,
it becomes clear that you must compute $\frac{\partial C}{\partial a_{j}^{L}}$.

\begin{align}
	\delta^{L}     & = \nabla_{a} \text{C} \circ \frac{\partial \phi}{\partial z}(z^{L})                            \\
	\delta_{j}^{L} & = \frac{\partial C}{\partial a_{j}^{L}} \frac{\partial \phi}{\partial z}(z_{j}^{L})            \\
	\delta_{j}^{L} & = \frac{\partial C}{\partial a_{j}^{L}}(a_{j}^{L}) \frac{\partial \phi}{\partial z}(z_{j}^{L})
\end{align}

Assuming $C$ is the cost for a single training vector $x$, then

\begin{align}
	C                                       & = \frac{1}{m} \sum_{j=0}^{m} { (a_{j}^{L} - y_j)^{2} } \\
	\frac{\partial C}{\partial a_{j}^{L}}   & = 2(a_{j}^{L} - y_j)                                   \\
	\frac{\partial C_x}{\partial a_{j}^{L}} & = 2(a_{j}^{L} - y_j)
\end{align}
and the partial derivative returns a scalar, but no summation is taken since
a summation over a vector would return a scalar, and the desired result of the gradient
is a vector.

The gradient must therefore be taken with respect to a certain parameter, otherwise
known as the Jacobian matrix, the partial derivative with respect
to each variable of a vector valued function, would result. To visualize what the gradient
vector with respect to the activation only looks like, consider the below vector.
...
$\theta^{L}$ are the relevant parameters and variables for the layer layer $L$.
Such a matrix would be as follows:
...
\begin{align}
	...
	\begin{bmatrix}
		...
		\vdots                                           & \vdots                                            & \dfrac{\partial C}{\partial W_{00}^{L}}         & \vdots                                                  & \vdots                                         \\
...
only consider what a function is. If you think of the $\frac{\partial C}{\partial a_{j}^{L}}$
as just another function, which it is in fact, then of what variables is it a function?
It is simple: $a_{j}^{L}$ and $y_j$! We \textit{know} the values of these variables
since we know the $a_{j}^{L}$ is just the output of a single neuron in a layer
of neurons. Moreover, $y_j$ is not something we have to compute, but rather it
is a known element of a vector of targets $y$, or desired outcomes, on which the error of the network
is based.
...
passed onto the next layer as the new input $X$ to the \codeword{__call__} method,
