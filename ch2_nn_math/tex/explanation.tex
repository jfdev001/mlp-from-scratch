...
observe that the activation vector $a^{l} \in \mathcal{R}^{n_a}$ is clearly not a matrix,
...
then derivative is known and shown below.
...
neurons is also easily computed. For the mean squared error function,
you would derive the function with respect to only a single activation. That is,
you are finding the rate of change of the cost function for an infinitesimally
small change in the activation.
This means that the summation and number of samples to divide by would be one.
This is important because the summation and division operation are removed
from the equation.
...
the network (i.e., you will use $\delta^{L}$ in the comptuation of $\delta^{L-1}$).
To validate the shapes of the operands relevant to the equation for $\delta^{l}$,
refer to the below statements.

\begin{align}
	W^{l+1}                                 & \in \mathcal{R}^{n_h^{l+1} \times n_{h}^{l}} \\
	\delta^{l+1}                            & \in \mathcal{R}^{n_h^{l+1}}                  \\
	\frac{\partial \phi}{\partial z}(z^{l}) & \in \mathcal{R}^{n_{h}^{l}}
\end{align}
